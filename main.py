...
async def send_scheduled_report(context: ContextTypes.DEFAULT_TYPE):
 ...
 await context.bot.send_message(chat_id=owner_id, text=result[chunk_start:chunk_start + 4096])
 for chunk_start in range(0, len(report_fish), 4096):
 await context.bot.send_message(chat_id=owner_id, text=report_fish[chunk_start:chunk_start + 4096])
 for chunk_start in range(0, len(report_veg), 4096):
 await context.bot.send_message(chat_id=owner_id, text=report_veg[chunk_start:chunk_start + 4096])
 for chunk_start in range(0, len(report_meat), 4096):
 await context.bot.send_message(chat_id=owner_id, text=report_meat[chunk_start:chunk_start + 4096])
 except Exception as e:
...
async def show_all_purchase_reports(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 if has_data:
 report_msg += f"\n✅ المجموع الكلي للمجهز: {format_float(grand_total_for_this_supplier)} دينار 💸"
...
def main():
 ...
 _schedule_daily_with_catchup(app, send_scheduled_report, REPORT_DAILY_HOUR, REPORT_DAILY_MINUTE, "daily_report", catchup_minutes=480)
 _schedule_daily_with_catchup(app, do_scheduled_reset, RESET_DAILY_HOUR, RESET_DAILY_MINUTE, "daily_reset", catchup_minutes=5)
 logger.info(
