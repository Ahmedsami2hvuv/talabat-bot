from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup from telegram.ext import ( ApplicationBuilder, ContextTypes, CommandHandler, MessageHandler, CallbackQueryHandler, ConversationHandler, filters ) import uuid

orders = {} pricing = {} current_product = {}

ASK_BUY, ASK_SELL = range(2) TOKEN = "7508502359:AAFtlXVMJGUiWaeqJZc0o03Yy-SgVYE_xz8"

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text("أرسل عنوان الطلب في السطر الأول، ثم المنتجات كل واحدة في سطر.")

async def receive_order(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.message.from_user.id lines = update.message.text.strip().split('\n') if len(lines) < 2: await update.message.reply_text("أرسل العنوان في السطر الأول، وكل منتج في سطر جديد.") return

title = lines[0]
products = lines[1:]
order_id = str(uuid.uuid4())[:8]  # ID مختصر للزر
...
await show_buttons(update.effective_chat.id, context, order_id)

async def show_buttons(chat_id, context, order_id): order = orders[order_id] buttons = [] for p in order["products"]: is_done = ( p in pricing[order_id] and 'buy' in pricing[order_id][p] and 'sell' in pricing[order_id][p] ) label = f"✅ {p}" if is_done else p buttons.append([InlineKeyboardButton(label, callback_data=f"{order_id}|{p}")])
...
if all(
    p in pricing[order_id] and 'buy' in pricing[order_id][p] and 'sell' in pricing[order_id][p]
...
app = ApplicationBuilder().token(TOKEN).build() app.add_handler(CommandHandler("start", start))
